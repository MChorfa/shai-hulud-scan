...
async function buildDatabase() {
  ...
  // Remove existing DB to ensure schema is recreated fresh
  ...
  const packages = db
    .prepare("SELECT id, name, version, description FROM packages")
    .all() as Array<{
    id: number;
    name: string;
    version: string;
  ...
  for (let i = 0; i < packages.length; i += batchSize) {
    ...
    const results =
      await embeddingsService.batchGeneratePackageEmbeddings(batch);

    const insertStmt = db.prepare(
      "UPDATE packages SET embedding = ? WHERE id = ?",
    );
    ...
    transaction(results);
    console.log(
      `   Processed ${Math.min(i + batchSize, packages.length)}/${packages.length}`,
    );
  }
  ...
  const count = db
    .prepare(
      "SELECT COUNT(*) as count FROM packages WHERE embedding IS NOT NULL",
    )
    .get() as { count: number };
